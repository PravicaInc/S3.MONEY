    ...
    <div className={styles.container}>
      ...
      <div style={{ width: "45%", zIndex: 10 }}>
        ...
        <div style={{ marginTop: "2rem", marginBottom: "5rem" }}>
          <Typography type={"d_xs-500"}>Choose the Account That Fits Your Needs</Typography>
        </div>
        <div style={{ marginBottom: "2rem" }}>
          <Typography type={"t_lg-400"}>Continue as: </Typography>
        </div>
        ...
        <Button onClick={handleGetStarted}>
          <Typography type={"t_sm-600"}>Get Started</Typography>
        </Button>
      </div>
...
const UserType: FC<IUserTypeProps> = ({ type, setSelectedUserType, selectedUserType }) => {
  ...
  const image = `../../assets/${type.split(" ").join("_").toLowerCase()}.svg?react`;
  ...
  return (
    <div className={`${styles.card} ${isActive && styles.active}`} onClick={handleClicked}>
      ...
      {Icon}
      <Typography style={{ color: "inherit" }} type={`t_xs-${isActive ? "600" : "400"}`}>
        {type}
      </Typography>
    </div>
  );
