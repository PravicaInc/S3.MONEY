module <%- packageName %>::<%- packageName %> {
    ...
    public struct EventPaused has copy, drop {}
    public struct EventUnpaused has copy, drop {}

    // Events: address frozen/unfrozen
    public struct EventTransfersFrozen has copy, drop { address: address }
    public struct EventTransfersUnfrozen has copy, drop { address: address }

    // Events: mint, burn, transfer, allocate
    ...
    public struct EventAllocation has copy, drop {
        sender: address,
        recipient: address,
        amount: u64,
    }
    ...
    public fun transfer<T>(policy: &TokenPolicy<T>, token: Token<T>, recipient: address, ctx: &mut TxContext) {
        ...
        let mut request = token::transfer<T>(token, recipient, ctx);
    ...
    // Marked "package" so the tests can call it.
    public(package) fun set_rules<T>(policy: &mut TokenPolicy<T>, cap: &TokenPolicyCap<T>, ctx: &mut TxContext) {
