...
export async function getHoldings(address: string, ticker: string, range: string) {
  ...
  switch (suffix) {
    case 'd':
      rangeStart = df.sub(new Date(), { days: amount }).toISOString()
        .slice(0, 10);
      break;
    case 'm':
      rangeStart = df.sub(new Date(), { months: amount }).toISOString()
        .slice(0, 7) + '-01';
      break;
    case 'y':
      rangeStart = df.sub(new Date(), { years: amount }).toISOString()
        .slice(0, 4) + '-01-01';
      break;
  }
  ...
  return (
    response.Items?.map(item => {
      const { date, holdings } = unmarshall(item);
